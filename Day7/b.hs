...
getNum (x:y:xs) (c:cs)
        | c == '+' = getNum (x+y:xs) cs
        | c == '*' = getNum (x*y:xs) cs
        | otherwise = getNum (concatNums x y:xs) cs

getAllCombinations :: [x] -> [[Char]]
getAllCombinations [] = [[]]
getAllCombinations (x:xs) = map (\c -> c ++ ['+']) (getAllCombinations xs) ++ map (\c -> c ++ ['*']) (getAllCombinations xs) ++ map (\c -> c ++ ['|']) (getAllCombinations xs)

concatNums x y = toInt (show x <> show y)
